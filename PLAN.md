...
> 상태: **1차 완료** — 1~6단계 완료 (화면 + Level 1~7 전체 315개 데이터, 영상 717개 재검증, 모바일 점검). 남은 것: 영상 없는 1개(Hit — Rod Howell 시범은 Vimeo에만 있고 YouTube에는 없음, 2026-09-11 확인), Pendulum Wing의 소리별 카운트(Rod Howell 시범 영상 자막은 `&a**1` = 5소리로 확인했으나 소리마다 어떤 동작인지는 미확정 — 설명에만 표기)
> 참고: 원 자료 표기에서 **대문자 = 왼발, 소문자 = 오른발** (강세 아님). 모든 레벨 설명이 이 규칙을 따름
...
- 🌙 다크 모드 — **완료 (2026-09-11)**: 머리글 버튼으로 System → Light → Dark 전환 (기본은 기기 설정). 색은 `src/index.css`의 CSS 변수로 관리, 레벨 색도 변수(Level 7은 다크에서 밝은 회색)
- ~~기록 내보내기·불러오기~~ — 필요 없음 (2026-09-11 결정)
